...
def _get_fallback_system_prompt() -> str:
    """Old system prompt content - now externalized to template file"""
    current_date = datetime.now().strftime("%B %d, %Y")
    return f"""The assistant is Claude, created by Anthropic.
...
Claude’s reliable knowledge cutoff date - the date past which it cannot answer questions reliably - is the end of January 2025. It answers all questions the way a highly informed individual in January 2025 would if they were talking to someone from {current_date}, and can let the person it’s talking to know this if relevant. If asked or told about events or news that occurred after this cutoff date, Claude can’t know either way and lets the person know this. If asked about current news or events, such as the current status of elected officials, Claude tells the user the most recent information per its knowledge cutoff and informs them things may have changed since the knowledge cut-off. Claude neither agrees with nor denies claims about things that happened after January 2025. Claude does not remind the person of its cutoff date unless it is relevant to the person’s message.
...
def load_agent_config(config_file: Optional[str] = None) -> AgentConfig:
...
def build_agent_config_from_server(
    server_config: "MCPServerConfig", api_key: Optional[str] = None
) -> AgentConfig:
    """
    ...
    """
    from .models import MCPServerConfig as AgentMCPServerConfig
    ...
    # Note: backend might send different types like 'user_deployed'
    agent_server = AgentMCPServerConfig(
        type=server_config.type,  # Accept any type from backend
        url=server_config.url,
        name=server_config.name,
        tool_configuration=server_config.tool_configuration,  # Can be dict or object
...
def save_config_template(filename: str = "config.json"):
