...
from src.test_mcp.testing.compliance.mcp_compliance_tester import (
    MCPComplianceTester,
    MCPComplianceTestResult,
    MCPServerInfo
)
from src.test_mcp.shared.result_models import TestType
from src.test_mcp.shared.progress_tracker import TestStatus
...
class TestMCPComplianceTester:
    ...
    @patch('src.test_mcp.testing.compliance.mcp_compliance_tester.ClientSession', None)
    @pytest.mark.asyncio
    ...
    @patch('src.test_mcp.testing.compliance.mcp_compliance_tester.ClientSession')
    @patch('src.test_mcp.testing.compliance.mcp_compliance_tester.streamablehttp_client')
    @pytest.mark.asyncio
    ...
    def test_new_compliance_categories(self):
        ...
        test_categories = ['handshake', 'capabilities', 'tools', 'resources', 'advanced']
...
def oauth_server_config():
    ...
    return {
        ...
        'oauth_config': {
            'required': True,
            'test_endpoints': True,
            'expected_scopes': ['mcp:read', 'mcp:write'],
...
class TestMCPComplianceIntegration:
    ...
    async def test_full_compliance_suite_no_oauth(self, server_config):
        ...
        with patch('src.test_mcp.testing.compliance.mcp_compliance_tester.ClientSession', None):
    ...
    async def test_error_handling_connection_failure(self, server_config):
        ...
        with patch('src.test_mcp.testing.compliance.mcp_compliance_tester.ClientSession'):
            tester = MCPComplianceTester(server_config)
            
            # Mock connection failure
            tester._connect_to_server = AsyncMock(side_effect=Exception("Connection failed"))
